...
                
                # This is the bug fix: `book` could be `None` if the ID doesn't exist
                if not book or not book["is_available"]:
                    return False

                # Insert transaction with a defined borrow and return date
                borrow_date = date.today()
                return_date = borrow_date + timedelta(weeks=2) # Example: 2 weeks borrow period
                
                cursor.execute(
                    """
...
            conn = get_connection()
            with conn.cursor() as cursor:
                # The is_returned=FALSE check is already there. This is good.
                cursor.execute("""
...
                    # This means no un-returned transaction was found for this user/book
...
            conn = get_connection()
            with conn.cursor() as cursor:
                # Join with books and users tables for more useful data
                cursor.execute("""
